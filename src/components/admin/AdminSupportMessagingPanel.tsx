    ...
    <div className="rounded-3xl border border-edge/80 bg-surface-elevated p-6 shadow-sm space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-edge/60 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-serif text-xl font-medium text-ink">Centro de Soporte y Mensajería</h3>
        ...
        <div className="flex items-center gap-1.5 bg-surface border border-edge/60 p-1 rounded-2xl self-start sm:self-auto">
          <button
            ...
            className={`px-3 py-1.5 text-xs font-medium rounded-xl transition cursor-pointer ${
              filterStatus === 'open' ? 'bg-accent text-surface shadow-xs' : 'text-ink-muted hover:text-ink'
            }`}
          >
            Pendientes
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-xs font-medium rounded-xl transition cursor-pointer ${
              filterStatus === 'closed' ? 'bg-accent text-surface shadow-xs' : 'text-ink-muted hover:text-ink'
            }`}
          >
            Resueltas
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-xs font-medium rounded-xl transition cursor-pointer ${
              filterStatus === 'all' ? 'bg-accent text-surface shadow-xs' : 'text-ink-muted hover:text-ink'
            }`}
          >
            Todas
          </button>
        ...
        <div className="md:col-span-5 lg:col-span-4 border border-edge/60 bg-surface/40 rounded-2xl overflow-hidden flex flex-col">
          <div className="p-3 border-b border-edge/60 bg-surface/80 flex items-center justify-between">
            <span className="text-xs font-medium uppercase tracking-wider text-ink-muted">
              Consultas Recibidas ({conversations.length})
            </span>
          </div>
              ...
              conversations.map((conv) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3.5 rounded-2xl transition cursor-pointer ${
                      isSelected
                        ? 'bg-accent/10 border border-accent/20 text-ink shadow-xs'
                        : 'hover:bg-surface border border-transparent text-ink-muted hover:text-ink'
                    }`}
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-semibold text-xs text-ink truncate">
                        {conv.authorName || 'Autor'}
                      </span>
                    ...
                    {conv.projectTitle && (
                      <p className="mt-0.5 text-[10px] text-accent font-light truncate">
                        Obra: {conv.projectTitle}
                      </p>
                    )}

                    {conv.lastMessage && (
                      <p className="mt-1.5 text-[11px] text-ink-muted/80 line-clamp-1">
          ...
          {!activeConv ? (
            ...
            <>
              ...
              <div className="p-4 border-b border-edge/60 bg-surface/80 flex items-center justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {activeConv.status === 'open' ? 'Pendiente' : 'Resuelta'}
                    </span>
                  </div>
                  <p className="text-xs text-ink-muted mt-0.5 font-light">
                    Autor: <span className="font-medium text-ink">{activeConv.authorName || 'Desconocido'}</span> ({activeConv.authorEmail || 'Sin email'})
                  </p>
                </div>
                ...
                >
                  {activeConv.status === 'open' ? 'Marcar Resuelta' : 'Reabrir Consulta'}
                </button>
              </div>
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-[10px] font-medium text-ink-muted">
                            {isAdmin ? 'Equipo Editorial' : activeConv.authorName || 'Autor'}
                          </span>
                        ...
                        <div
                          className={`max-w-[85%] rounded-2xl px-4 py-3 text-xs leading-relaxed ${
                            isAdmin
                              ...
                              : 'bg-surface border border-edge/80 text-ink rounded-bl-xs'
                          }`}
                        >
              ...
              <form
                ...
                className="p-3 border-t border-edge/60 bg-surface/90 flex items-center gap-2"
              >
                <input
                  type="text"
                  value={replyBody}
                  onChange={(e) => setReplyBody(e.target.value)}
                  placeholder="Escribe la respuesta oficial para el autor..."
                  className="flex-1 rounded-2xl border border-edge/80 bg-surface px-4 py-2.5 text-xs text-ink placeholder:text-ink-muted/50 focus:border-accent focus:outline-none"
                />
                <button
                  type="submit"
                  disabled={sending || !replyBody.trim()}
                  className="flex h-10 px-4 items-center justify-center gap-2 rounded-2xl bg-accent text-surface font-medium text-xs transition hover:bg-accent-hover disabled:opacity-40 cursor-pointer shrink-0"
                >
                  {sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                  <span>Responder y Notificar</span>
                </button>
              </form>
            </>
          )}
