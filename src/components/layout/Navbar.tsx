        ...
        <div className="flex items-center gap-2.5 sm:gap-4">
                  ...
                  >
                    <div className="flex items-center justify-between border-b border-edge/60 pb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-semibold uppercase tracking-wider text-ink">Notificaciones</span>
                      </div>
                      {unreadCount > 0 ? (
                        <button type="button" onClick={handleMarkAllRead} className="flex items-center gap-1 text-[10px] font-medium text-accent hover:underline cursor-pointer">
                          <CheckCheck className="h-3 w-3" />
                          <span>Marcar leídas</span>
                        </button>
                      ) : (
                        <span className="rounded-full bg-accent/10 px-2 py-0.5 text-[10px] font-mono font-medium text-accent">Al día</span>
                      )}
                    </div>
                    ...
                    <div className="mt-3 space-y-2.5 max-h-72 overflow-y-auto pr-0.5">
                      {activeNotifications.length === 0 ? (
                        <div className="py-6 text-center text-xs text-ink-muted">
                          ...
                          <p className="text-[11px] font-light mt-1 text-ink-muted/80">Las actualizaciones de tu obra y avisos editoriales aparecerán aquí en tiempo real.</p>
                        </div>
                      ) : (
                          ...
                          return (
                            <button
                              ...
                              className={`group relative flex w-full items-start gap-3 rounded-xl border p-3 text-left text-xs transition cursor-pointer ${
                                isUnread ? 'border-accent/40 bg-accent/5' : 'border-edge/50 bg-surface/50 opacity-80'
                              }`}
                            >
                              <div className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${isUnread ? 'bg-accent/20 text-accent' : 'bg-surface text-ink-muted'}`}>
                                <CheckCircle2 className="h-3.5 w-3.5" />
                              </div>
                              <div className="space-y-1 min-w-0 flex-1">
                                <div className="flex items-center justify-between gap-1">
                                  <p className="font-medium text-ink truncate">{n.title}</p>
                                  {isUnread && <span className="h-1.5 w-1.5 rounded-full bg-accent shrink-0" />}
                                </div>
                                <p className="text-[11px] font-light text-ink-muted leading-relaxed line-clamp-2">{n.message}</p>
                                {n.createdAt && (
                    ...
                    <div className="mt-3 border-t border-edge/40 pt-2 text-center">
                      <span className="text-[10px] text-ink-muted/80">Studio Flamkit & Art • Canal Oficial</span>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
          ...
          {loading ? (
            ...
            <div className="relative" ref={dropdownRef}>
              <button type="button" onClick={() => setDropdownOpen(!dropdownOpen)} className="flex items-center gap-2 rounded-full border border-edge/50 bg-surface-elevated py-1.5 px-3 sm:px-3.5 text-xs font-medium text-ink hover:border-accent hover:bg-surface transition cursor-pointer select-none active:scale-95" aria-label="Menú de perfil">
                <div className="flex h-6 w-6 items-center justify-center rounded-full bg-accent/15 text-accent"><User className="h-3.5 w-3.5" /></div>
              ...
              <AnimatePresence>
                {dropdownOpen && (
                  <motion.div initial={{ opacity: 0, y: 8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.95 }} transition={{ duration: 0.15, ease: 'easeOut' }} className="absolute right-0 mt-2 w-64 origin-top-right rounded-2xl border border-edge/60 bg-surface-elevated p-2 shadow-xl ring-1 ring-black/5 z-50">
                    <div className="px-3 py-2.5 mb-1.5 border-b border-edge/60 text-left">
                      <p className="text-xs font-semibold text-ink truncate leading-none">{displayName}</p>
                      <p className="mt-1 text-[10px] text-ink-muted truncate">{displayEmail}</p>
                      <div className="mt-2.5 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider bg-accent/10 text-accent">
                        {role === 'admin' ? (<><ShieldCheck className="h-2.5 w-2.5" />Administrador</>) : (<><BookOpen className="h-2.5 w-2.5" />Autor</>)}
                      </div>
                    </div>
                    ...
                    <div className="space-y-0.5">
                      ...
                      <Link href={routes.home} onClick={() => setDropdownOpen(false)} className="flex items-center gap-2.5 rounded-xl px-3 py-2 text-xs font-medium text-ink hover:bg-surface transition"><Home className="h-3.5 w-3.5 text-ink-muted" /><span>Volver a Inicio</span></Link>
                      <div className="h-px bg-edge/60 my-1.5" />
                      <div className="px-3 py-1 text-[10px] uppercase font-bold tracking-wider text-ink-muted select-none">Explorar Estudio</div>
                      {links.map((link) => (
                        <Link key={link.label} href={link.href} onClick={() => setDropdownOpen(false)} className="flex items-center gap-2.5 rounded-xl px-3 py-2 text-xs font-medium text-ink-muted hover:text-ink hover:bg-surface transition"><Compass className="h-3.5 w-3.5" /><span>{link.label}</span></Link>
                      ))}
                      <div className="h-px bg-edge/60 my-1.5" />
                      <button type="button" onClick={handleLogout} className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-xs font-medium text-rose-600 dark:text-rose-400 hover:bg-rose-500/10 hover:text-rose-700 transition text-left cursor-pointer"><LogOut className="h-3.5 w-3.5" /><span>Cerrar sesión</span></button>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            <div className="flex items-center gap-3">
              <Link href={routes.dashboard} className="hidden text-xs font-semibold text-ink-muted hover:text-ink transition sm:inline mr-1">Centro de Autor</Link>
              <Link href={routes.login} className="inline-flex"><Button variant="primary" className="text-xs py-1.5 px-4 h-9">Iniciar sesión</Button></Link>
            </div>
          )}
        </div>
      </nav>
