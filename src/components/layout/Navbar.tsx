...
import { getUserNotifications, markNotificationAsRead, markAllNotificationsAsRead, type NotificationRow } from '@/services/notification.service';
...
export function Navbar() {
  ...
  const unreadCount = activeNotifications.filter((n) => n.status === 'unread' || n.status === 'pendiente').length;
    ...
    <header className="sticky top-0 z-20 border-b border-edge bg-surface/85 backdrop-blur-md">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-8">
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          {user && isDashboardRoute && (
            <div className="relative" ref={notifRef}>
              <button
                ...
                className="relative flex h-9 w-9 items-center justify-center rounded-full border border-edge bg-surface-elevated text-ink-muted hover:border-accent hover:text-accent transition cursor-pointer"
              ...
              <AnimatePresence>
                {notificationsOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 mt-2 w-80 origin-top-right rounded-2xl border border-edge/80 bg-surface-elevated p-4 shadow-xl ring-1 ring-black/5 z-30"
                  >
                    ...
                    <div className="mt-3 space-y-2.5 max-h-72 overflow-y-auto pr-0.5">
                      {activeNotifications.length === 0 ? (
                        ...
                        activeNotifications.map((n) => {
                          const isUnread = n.status === 'unread' || n.status === 'pendiente';
                          return (
                            <div
                              key={n.id}
          ...
          {loading ? (
            <div className="h-8 w-8 rounded-full border border-edge/50 animate-pulse bg-surface-elevated" />
          ) : user ? (
            ...
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="flex items-center gap-2 rounded-full border border-edge/50 bg-surface-elevated py-1.5 px-3.5 text-xs font-medium text-ink hover:border-accent hover:bg-surface transition cursor-pointer select-none"
              >
              ...
              <AnimatePresence>
                {dropdownOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 8, scale: 0.95 }}
                    transition={{ duration: 0.15, ease: 'easeOut' }}
                    className="absolute right-0 mt-2 w-64 origin-top-right rounded-2xl border border-edge/50 bg-surface-elevated p-2 shadow-xl ring-1 ring-black/5 z-30"
                  >
                    {/* Header profile info */}
