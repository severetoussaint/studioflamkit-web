...
const FALLBACK_PHASE_BY_STATE: Record<'none' | 'pending' | 'proposal' | 'active' | 'rejected', EditorialPhase | null> = {
  none: null,
  pending: 'analysis',
  proposal: 'proposal',
  active: 'production',
...
export function ProgressTimeline({ steps, currentState, journey = null }: ProgressTimelineProps) {
  ...
  const proposalIsActive = journey?.currentPhase === 'proposal' || currentState === 'proposal';
    ...
    <div className="relative overflow-hidden rounded-3xl border-edge/50 bg-surface-elevated/95 p-5 sm:p-6 shadow-[0_12px_36px_rgba(0,0,0,0.20)] backdrop-blur-xs">
      ...
      <div className="relative flex items-start justify-between gap-1 sm:gap-2">
        {activeSteps.map((step, index) => {
          ...
          const isProposal = step.id === 'proposal';
          const clickable = isProposal && proposalIsActive && isActive;
          return (
            <React.Fragment key={step.id}>
              <button
                ...
                disabled={!clickable}
                className={`relative flex min-w-0 flex-1 flex-col items-center gap-2 text-center ${clickable ? 'cursor-pointer' : 'cursor-default'}`}
              >
                <span className={`flex h-9 w-9 items-center justify-center rounded-full border text-xs font-semibold transition ${
                  isCompleted ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-600' :
                  isActive ? 'border-accent/40 bg-accent/10 text-accent shadow-sm' :
                  isBlocked ? 'border-edge/60 bg-surface text-ink-muted/50' :
                  'border-edge/60 bg-surface text-ink-muted'
                }`}>
                  {isCompleted ? <Check className="h-4 w-4" /> : isBlocked ? <Lock className="h-3.5 w-3.5" /> : index + 1}
                </span>
                <span className={`text-[10px] leading-4 sm:text-[11px] ${isActive ? 'font-semibold text-ink' : 'text-ink-muted'}`}>{step.title}</span>
              </button>
              {index < activeSteps.length - 1 && (
                <div className={`mt-4 h-px flex-1 ${isCompleted ? 'bg-emerald-500/30' : 'bg-edge/50'}`} />
              )}
            </React.Fragment>
          );
        })}
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        <div className="rounded-2xl border border-edge/50 bg-surface/60 p-3">
          <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-wide text-ink-muted"><Clock className="h-3.5 w-3.5 text-accent" /> Estado</div>
          <p className="mt-1 text-xs text-ink">{activeIndex >= 0 ? activeSteps[activeIndex].title : 'Pendiente'}</p>
        </div>
        <div className="rounded-2xl border border-edge/50 bg-surface/60 p-3">
          <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-wide text-ink-muted"><Disc className="h-3.5 w-3.5 text-accent" /> Siguiente</div>
          <p className="mt-1 text-xs text-ink">{activeIndex >= 0 && activeIndex + 1 < activeSteps.length ? activeSteps[activeIndex + 1].title : 'Continuar'}</p>
        </div>
        <div className="rounded-2xl border border-edge/50 bg-surface/60 p-3">
          <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-wide text-ink-muted"><Sparkles className="h-3.5 w-3.5 text-accent" /> Acción</div>
          <p className="mt-1 text-xs text-ink">{proposalIsActive ? 'Revisar cuando esté disponible' : activeIndex >= 0 ? 'Esperar siguiente etapa' : 'Enviar manuscrito'}</p>
        </div>
      </div>
    </div>
  );
