...
export function StatusHero({ state, projectTitle, submittedDate, progress = 0, statusLabel, journey, onUploadClick, onViewFilesClick, onToggleCarousel, onOpenBriefClick }: StatusHeroProps) {
  if (state === 'none') {
    return (
  ...
  if (state === 'rejected') {
    return (
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }} className="relative overflow-hidden rounded-3xl border border-rose-500/25 bg-surface-elevated/95 p-5 sm:p-8 md:p-12 shadow-[0_12px_40px_rgba(0,0,0,0.03)]">
  ...
  if (state === 'pending') {
    return (
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }} className="relative overflow-hidden rounded-3xl border-amber-500/25 bg-surface-elevated/95 p-5 sm:p-8 md:p-12 shadow-[0_12px_40px_rgba(0,0,0,0.03)]">
  ...
  if (journey?.currentPhase === 'proposal') {
    return (
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }} className="relative overflow-hidden rounded-3xl border border-amber-500/25 bg-surface-elevated/95 p-5 sm:p-8 md:p-12 shadow-[0_12px_40px_rgba(0,0,0,0.03)]">
        ...
        <div className="relative z-10 max-w-3xl">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-amber-500/30 bg-amber-500/10 px-3.5 py-1 text-[11px] font-semibold uppercase tracking-[0.15em] text-amber-800 dark:text-amber-300"><FileText className="h-3.5 w-3.5" />Propuesta en preparación</div>
          ...
          <p className="mt-4 text-sm sm:text-base leading-relaxed text-ink-muted font-light">El análisis terminó y Studio FLAMKIT está preparando la oferta comercial y técnica. Todavía no estás en producción.</p>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }} className="relative overflow-hidden rounded-3xl border-edge/50 bg-surface-elevated/95 p-5 sm:p-8 md:p-12 shadow-[0_12px_40px_rgba(0,0,0,0.03)]">
      ...
      <div className="relative z-10 flex flex-col justify-between gap-8 lg:flex-row lg:items-center">
        ...
        <div className="flex shrink-0 flex-col items-start rounded-2xl border-edge/50 bg-surface/80 p-6 sm:min-w-[240px] shadow-2xs backdrop-blur-xs"><div className="flex items-center justify-between w-full mb-3"><span className="text-[10px] font-medium uppercase tracking-[0.22em] text-ink-muted">Avance General</span><span className="font-serif text-xl font-semibold text-accent">{effectiveProgress}%</span></div><div className="h-2 w-full rounded-full bg-edge/60 overflow-hidden"><motion.div className="h-full rounded-full bg-accent" initial={{ width: 0 }} animate={{ width: `${effectiveProgress}%` }} transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }} /></div><div className="mt-4 flex items-center gap-2 text-xs text-ink-muted/90"><Headphones className="h-3.5 w-3.5 text-accent shrink-0" /><span>Cabina de producción activa</span></div></div>
      </div>
    </motion.div>
  );
}
