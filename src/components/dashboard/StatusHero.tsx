...
import type { EditorialJourney, Project } from '@/types/domain.types';

/**
 * Presentation contract for StatusHero.
 * Domain data (journey, project) is mapped to presentation values internally.
 * Progress remains a presentation prop since EditorialJourney does not contain progress.
 */
interface StatusHeroProps {
  ...
  progress?: number;
  statusLabel?: string;
  /** Domain editorial journey from AuthorProjectViewModel */
  journey?: EditorialJourney | null;
  /** Domain project from AuthorProjectViewModel (optional, used for fallback title) */
...
export function StatusHero({
  state,
  projectTitle: explicitProjectTitle,
  submittedDate,
  progress = 0,
  statusLabel,
  ...
  const snapshot = snapshotFromDashboardProps({
    ...
    projectTitle: explicitProjectTitle ?? null,
    submittedDate: submittedDate ?? null,
  });

  storeEditorialSnapshot(snapshot);
  const legacyJourney = resolveEditorialJourney(snapshot);

  // Use domain journey when available, otherwise fall back to legacy resolution.
  // This maintains visual behavior when journey === null.
  const effectiveJourney = domainJourney ?? legacyJourney;

  // Project title: prefer explicit prop, then domain project (not yet linked in this migration),
  // then legacy resolution.
  const projectTitle = explicitProjectTitle ?? undefined;
      ...
      >
        ...
        <div className="relative z-10 max-w-3xl">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border-amber-500/30 bg-amber-500/10 px-4 py-1.5 text-[11px] font-medium tracking-wide text-amber-800 dark:text-amber-300">
            ...
            <span className="uppercase tracking-[0.16em]">{effectiveJourney.label === 'Sin manuscrito' ? 'Manuscrito Recibido' : effectiveJourney.label}</span>
          </div>
          ...
          <p className="mt-4 text-base leading-relaxed text-ink-muted font-light">
            ...
            {effectiveJourney.nextActionDescription}
          </p>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-4">
            ...
            <div className="inline-flex items-center gap-2 rounded-2xl bg-surface/80 px-4 py-2.5 text-xs text-ink-muted border-edge/60">
              ...
              <span>Siguiente paso: {effectiveJourney.nextActionTitle}</span>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <div className="relative z-10 flex flex-col justify-between gap-8 lg:flex-row lg:items-center">
        <div className="max-w-2xl">
          <div className="mb-4 flex flex-wrap items-center gap-3">
            <span className="inline-flex items-center gap-1.5 rounded-full border-emerald-500/25 bg-emerald-500/10 px-3.5 py-1 text-[11px] font-medium text-emerald-800 dark:text-emerald-300">
              ...
              <span className="uppercase tracking-[0.15em]">{effectiveJourney.label || statusLabel || 'En Producción Audiocinematográfica'}</span>
            </span>
        ...
        <div className="flex shrink-0 flex-col items-start rounded-2xl border-edge/50 bg-surface/80 p-6 sm:min-w-[240px] shadow-2xs backdrop-blur-xs">
          <div className="flex items-center justify-between w-full mb-3">
            ...
            <span className="font-serif text-xl font-semibold text-accent">{effectiveJourney.progress}%</span>
          </div>
          ...
          <div className="h-2 w-full rounded-full bg-edge/60 overflow-hidden">
            <motion.div
              ...
              initial={{ width: 0 }}
              animate={{ width: `${effectiveJourney.progress}%` }}
