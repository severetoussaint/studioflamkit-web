...
import { listAuthorProjectsData, type AuthorProjectOverview } from '@/services/project.service';
  ...
  useEffect(() => {
    ...
    async function loadAuthorContext() {
      ...
      try {
        ...
        const projects = await listAuthorProjectsData(user.id);

        if (!mounted) return;
        setRequestContext(context);
        setRequestState(context.state);
        setAuthorProjects(projects);
        setSelectedManuscriptId((current) => current ?? context.activeManuscriptId ?? context.manuscriptId ?? context.manuscripts[0]?.id ?? null);
      } catch (error) {
